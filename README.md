...
**Сборка запускается только вручную.** Автотриггеров нет: ни webhook, ни
периодического сканирования. Коммит в `main` сам по себе ничего не собирает
и никуда не деплоит.

Как запустить: Jenkins → `hexlet/Hexlet.Meetly` → ветка → **Build Now**.
Новую ветку сначала надо показать Jenkins — **Scan Repository Now** на уровне
задачи, иначе её job не появится.

Webhook не используется намеренно: хост Jenkins закрыт whitelist-фильтром
провайдера, подсети GitHub в него не входят, и события не доходят. Открывать
периметр ради этого не стали.
